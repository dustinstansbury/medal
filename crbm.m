...
methods
	...
	% CALCULATE HIDDEN EXPECTATIONS GIVEN VISIBLE UNITS
		...
		eHid = I./(1 + self.pool(I)); % (Eq 5.3; Lee, 2010)
		self.eHid = eHid;
	end
	...
	% CALCULATE VISIBLE EXPECTATIONS GIVEN EACH HIDDEN FEATURE MAP
		...
		for iK = 1:self.nFeats
			I(:,:,iK) = conv2(hid(:,:,iK),self.W(:,:,iK),'full');
		end
		I = sum(I,3) + self.b;
	...
	function self = init(self,args,data)
		...
		self.log.error = zeros(1,self.nEpoch);
		self.log.eta = self.log.error;
	end
	...
	% PRINT LEARNING PROGRESS
		if iE > 1
			if self.log.error(iE) > self.log.error(iE-1) & iE > 1
				indStr = '(UP)    ';
			else
				indStr = '(DOWN)  ';
			end
