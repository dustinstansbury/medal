...
properties
	...
	netInput = [];			% CURRENT INPUT TO NETWORK
	...
	crossValidating=0;		% ARE WE CROSSVALIDATIN NOW?
	...
	function self = train(self,X,targs); % MAIN LEARNING ALGORITHM
		...
		while 1
			...
			for iB = 1:nBatches
				...
				self.netInput = X(batchIdx,:);
				% PROPAGATE SIGNALS UP...
				self = self.prop(self.netInput);
				% CALCULATE COST...
			...
			if ~isempty(self.xValIdx)
				self = self.crossValidate(X,targs);
				self = self.assessNet;
