...
properties
	...
	anneal = 100;		% # OF EPOCHS TO START SIMULATED ANNEALING
	varyEta = 100;		% # OF EPOCHS TO START LEARNING RATE DECREAS
...
methods
	...
	function self = train(self)
		...
		for iE = 1:self.nEpoch
			...
			% SPARSITY
			if self.sparsity
				dcSparse = -self.eta*self.sparseFactor*(mean(self.pHid)-self.sparsity);
		   ...
		   if self.varyEta
				self.eta = self.eta/max(1,iE/self.varyEta); 
			end
